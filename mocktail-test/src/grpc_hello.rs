...
mod tests {
    ...
    async fn test_hello_unary() -> Result<(), anyhow::Error> {
        ...
        mocks.mock(|when, then| {
            ...
            then.pb(HelloResponse { message: "Hello Dan!".into() });
        });

        mocks.mock(|when, then| {
            when.path("/example.Hello/HelloUnary")
                .pb(HelloRequest { name: "Error".into() });
            then.internal_server_error().message("ugh");
            // alt: then.error(StatusCode::INTERNAL_SERVER_ERROR, "ugh");
        });


        let server = MockServer::grpc("example.Hello").with_mocks(mocks);
        server.start().await?;
        ...
        dbg!(&result);
        assert!(
            result.is_err_and(|e| { e.code() == tonic::Code::Internal })
        );
    ...
    #[test_log::test(tokio::test)]
    async fn test_hello_server_streaming() -> Result<(), anyhow::Error> {
        let mut mocks = MockSet::new();
        mocks.mock(|when, then| {
            when.path("/example.Hello/HelloServerStreaming")
                .pb(HelloRequest { name: "Dan, Gaurav".into() });
            then.pb_stream([
                HelloResponse { message: "Hello Dan!".into() },
                HelloResponse { message: "Hello Gaurav!".into() },
            ]);
        });
